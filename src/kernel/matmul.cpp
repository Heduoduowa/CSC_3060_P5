...
static void scalar_matmul_4row(
    ...
    for (; i + 3 < n; i += 4) {
        ...
        int k = 0;
        for (; k + 3 < n; k += 4) {
            const float av00 = a0[k + 0];
            const float av01 = a0[k + 1];
            const float av02 = a0[k + 2];
            const float av03 = a0[k + 3];

            const float av10 = a1[k + 0];
            const float av11 = a1[k + 1];
            const float av12 = a1[k + 2];
            const float av13 = a1[k + 3];

            const float av20 = a2[k + 0];
            const float av21 = a2[k + 1];
            const float av22 = a2[k + 2];
            const float av23 = a2[k + 3];

            const float av30 = a3[k + 0];
            const float av31 = a3[k + 1];
            const float av32 = a3[k + 2];
            const float av33 = a3[k + 3];

            const float* __restrict__ b0 = B + static_cast<std::size_t>(k + 0) * n;
            const float* __restrict__ b1 = B + static_cast<std::size_t>(k + 1) * n;
            const float* __restrict__ b2 = B + static_cast<std::size_t>(k + 2) * n;
            const float* __restrict__ b3 = B + static_cast<std::size_t>(k + 3) * n;
            ...
            for (int j = 0; j < n; ++j) {
                ...
                c0[j] += av00 * bv0 + av01 * bv1 + av02 * bv2 + av03 * bv3;
                c1[j] += av10 * bv0 + av11 * bv1 + av12 * bv2 + av13 * bv3;
                c2[j] += av20 * bv0 + av21 * bv1 + av22 * bv2 + av23 * bv3;
                c3[j] += av30 * bv0 + av31 * bv1 + av32 * bv2 + av33 * bv3;
            }
        }
