...
#ifndef BONUS_NORMALIZED_GM
#define BONUS_NORMALIZED_GM 0
#endif
...
#if BONUS_NORMALIZED_GM
              << std::setw(16) << "Bonus Mult" << '\n';
#else
...
#if GEOMETRIC_MEAN
    std::vector<double> geometric_mean_speedups;
    geometric_mean_speedups.reserve(10);
    bool gm_enable = true;
#endif
...
#if BONUS_NORMALIZED_GM
        const double gm_speedup =
            calculate_adjusted_naive_speedup(bench, naive_time, stu_time);
#else
        const double gm_speedup = baseline_speedup;
#endif
...
#if GEOMETRIC_MEAN
        geometric_mean_speedups.push_back(gm_speedup);
#endif
    };
...
#if GEOMETRIC_MEAN
    if (gm_enable && !geometric_mean_speedups.empty()) {
        const double geometric_mean_speedup =
            calculate_geometric_mean_speedup(geometric_mean_speedups);
